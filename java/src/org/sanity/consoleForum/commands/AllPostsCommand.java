...
public class AllPostsCommand implements Command {
    ...
    private static final String PostFormatOutput = "[%s] %s";
    ...
    public void execute() throws IOException {
        List<Post> postsFromDb = this.getEntityManager().getPosts()
                .stream()
        ...
        else
        {
            for (int i = 0; i < 3 && i < postsFromDb.size(); i++)
            {
                ...
                this.consoleWriter.write(Constants.CONSOLE_FORUM_OUTPUT_PREFIX);
                this.consoleWriter.importantLine(PostFormatOutput, post.getId(), post.getTitle());
            }
